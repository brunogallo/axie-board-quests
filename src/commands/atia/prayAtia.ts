...
import { atiaAbi } from "./atiaABI.js";
...
export async function checkBlessings(): Promise<void> {
  const keys = await getWallets('keys');
  log.subheader(`\n⚙️ Starting the daily pray (${keys.length} wallets)`);
  ...
  for (const key of keys) {
    ...
    if (key.delegateeAddresses.length > 5) {
      log.error(`Too much delegatees for prayer ${chalk.gray(signer.address.slice(-4))}`);
      return;
    }
