...
export async function cocochocoQuests(): Promise<void> {
  log.subheader(`\n🧰 CLAIM: 🍫 Feed 1 Cocochoco to any axie that you own`);
  ...
  for (const account of accounts.Axie) {
    ...
    const questTypes = [
      {
        type: "FeedCocoOwnedAxie",
        variant: "0",
        userAddress: account.walletAddress,
      },
    ];
